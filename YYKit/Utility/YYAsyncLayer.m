...
#define MAX_QUEUE_COUNT 16
  ...
  dispatch_once(&onceToken, ^{
    ...
    queueCount = queueCount < 1                 ? 1
                 : queueCount > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT
                                                : queueCount;
    if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
      ...
      for (NSUInteger i = 0; i < queueCount; i++) {
        queues[i] = dispatch_queue_create("com.ibireme.yykit.render",
      ...
      if (opaque) {
        CGContextSaveGState(context);
        {
          if (!backgroundColor || CGColorGetAlpha(backgroundColor) < 1) {
            ...
            CGContextAddRect(context, CGRectMake(0, 0, size.width * scale,
                                                 size.height * scale));
    ...
    if (self.opaque) {
      ...
      CGContextSaveGState(context);
      {
        if (!self.backgroundColor ||
            CGColorGetAlpha(self.backgroundColor) < 1) {
